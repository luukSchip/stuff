...
var timeEvents = [
    ...
    {time: 42.72, action: function(){createjs.Tween.get(things["rings.json"].model.position).to({y:-8}, 300, createjs.Ease.getPowIn);
    createjs.Tween.get(things["bassbell.json"].model.position).to({y:-8},300, createjs.Ease.getPowIn);
    createjs.Tween.get(things["DRUM3.json"].model.position).to({y:-8},300, createjs.Ease.getPowIn);
    var thing = things["DRUM3.json"]; 
        for(var i = 0; i < thing.clones.length; i++){
            var clone = thing.clones[i];
            createjs.Tween.get(clone.model.position).to({y:-8},300, createjs.Ease.getPowIn);
        };
